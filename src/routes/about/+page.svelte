		...
		<ol class="list-decimal [&>li]:ml-6 flex flex-col gap-y-4">
			...
			<li>
				...
				<div class="flex flex-col gap-y-4 ml-4 mt-4">
					...
					towers themselves do not move, they only increase range of mobility of the top piece <br
					/><br />
					b. Attack - if an enemy piece occupies a square within your range of movement, you can attack;
					if there is only one piece, your piece must then move into and occupy that square but if you’re
					attacking a tower, the result is that the tower loses the top and your piece now controls it
					<br /><br />
					c. Stack - if either a friendly piece or an enemy piece is within your range of movement, you
					can stack on top of that piece to form a tower (the top piece of the tower controls the tower
					regardless of pieces of a different team below it) <br /><br />
					d. Place - if you have less than the maximum pieces on the board, you may take a piece from
					your remaining pieces (captured pieces cannot be used) and place it anywhere on the board with
					following exceptions:
					<ol class="list-decimal ml-10 flex-col flex gap-y-4">
						<li>Multiple pawns cannot be placed in the same file</li>
						<li>
							Pawns cannot place the opposing Marshal into check mate (although check is allowed)
						</li>
						<li>
							Pieces cannot be placed within the first three ranks of the opposing team's side of
							the board
						</li>
					</ol>
				</div>
